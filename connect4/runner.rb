...
module Connect4
  class Runner
    ...
    def handle_move(player:, move:)
      ...
      if move == 'ai'
        ...
        unless issue.user.login != @user || issue.title.end_with?('new') || issue.title.end_with?('ai')
          ...
          "(https://github.com/russormes/russormes/issues/new?title=connect4%7Cdrop%7C#{player}%7Cai&body=Just+push+%27Submit+new+issue%27.+You+don%27t+need+to+do+anything+else.)."
    ...
    def handle_new_game
      if game.over? || @user.downcase == 'russormes'
        # red_team = []
        # blue_team = []
